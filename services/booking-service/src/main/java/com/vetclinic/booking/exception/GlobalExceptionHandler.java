...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<SlotFullyBookedErrorResponse> handleSlotFullyBooked(SlotFullyBookedException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(new SlotFullyBookedErrorResponse(Instant.now(), HttpStatus.CONFLICT.value(),
                        ex.getMessage(), ex.getSuggestions()));
    }
    ...
    public record SlotFullyBookedErrorResponse(Instant timestamp, int status, String message,
                                                List<SuggestedSlotResponse> suggestedTimes) {
    }
}
