...
public class MedicalRecordService {
    ...
    public MedicalRecordResponse createOrUpdateMedicalRecord(UUID appointmentId, MedicalRecordRequest request) {
        ...
        MedicalRecord record = medicalRecordRepository.findByAppointmentId(appointmentId)
                .orElseGet(() -> MedicalRecord.builder().appointment(appointment).build());
    ...
    // Staff tiếp nhận đơn thuốc sau khi bác sĩ kê xong — idempotent, gọi lại khi đã RECEIVED
    // thì không lỗi, chỉ trả về đúng trạng thái hiện tại.
    @Transactional
    ...
    // Hàng đợi cho staff: toàn bộ đơn thuốc đang chờ tiếp nhận, cũ nhất trước.
    @Transactional(readOnly = true)
    public List<MedicalRecordResponse> listPendingPrescriptions() {
        return medicalRecordRepository.findByStatusOrderByCreatedAtAsc(PrescriptionStatus.PENDING).stream()
                .map(this::toResponse)
                .toList();
    }
