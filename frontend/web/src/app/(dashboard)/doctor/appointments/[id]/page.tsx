...
import { Button, ErrorState, Spinner, StatusTag, TextAreaField, useToast } from "@/components/ui";
import type { AppointmentDetail, MedicalRecord, PrescriptionItemRequest } from "@/types";

/**
 * Màn hình khám. Sắp theo đúng thứ tự bác sĩ làm việc: xem con vật → đọc lý do khám →
 * ghi chẩn đoán → kê thuốc. Mỗi bước một khối riêng thay vì một biểu mẫu dài, để lúc
 * đang khám nhìn phát là biết đang ở đâu.
 */
export default function DoctorAppointmentPage() {
  ...
  return (
    <>
      <Link href="/doctor/appointments" className="text-sm text-moss underline underline-offset-2">
      ...
      <div className="mt-3 grid gap-5 lg:grid-cols-[20rem_1fr]">
        ...
        {/*
          ...
          và cũng không có nguy cơ ghi đè lúc đang gõ dở.
        */}
        <RecordEditor
    ...
    <aside className="space-y-4 lg:sticky lg:top-6 lg:self-start">
      ...
      {record && (
        <section className="rounded-[var(--radius-control)] border border-line bg-surface p-4">
          ...
          <StatusTag status={PRESCRIPTION_STATUS[record.status]} />
...
function RecordEditor({
  ...
  const [items, setItems] = useState<PrescriptionItemRequest[]>(
    (record?.prescriptionItems ?? []).map((it) => ({
  ...
  const locked = record != null && record.status !== "PENDING";

  const save = useMutation({
    mutationFn: () =>
      medicalRecordApi.save(appointmentId, {
        diagnosis,
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["medical-record", appointmentId] });
    ...
    onError: (err) => toast.error(err instanceof ApiError ? err.message : "Không lưu được bệnh án."),
  });
  ...
  return (
    <div className="space-y-5">
      ...
      <Section title="Kê đơn" hint="Lưu bệnh án kèm thuốc sẽ tự tạo phiếu thu tiền ở quầy">
      ...
      {locked ? (
        ...
        <div className="flex items-center gap-3">
          <Button
            ...
            onClick={() => save.mutate()}
          >
            {record ? "Lưu thay đổi" : "Lưu bệnh án"}
          </Button>
